...
// Inserting in Table(alumni_user) of Database(alumni)
	...
	function get_story1()
	{
		...
		$sql = $this->db->query("select * from views where status=1 order by modify_date desc limit 4 ");
		return $sql->result();
	}
	
	function view_detail($id)
	{
		$this->db->select('views.view_id,
		views.name,
		views.photo_path,
		views.detail,
		views.modify_date,
		...
		');		
		$this->db->from('campus');
		$this->db->join('views', 'campus.campus_id=views.campus_id');	
		$this->db->join('department', 'views.department_id = department.department_id');				
		$this->db->where('status','1');		
		$this->db->where('view_id',$id);
		$query = $this->db->get();
		return $query->result();
