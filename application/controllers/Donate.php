...
class Donate extends CI_Controller {
	...
	{		
		...
		$this->form_validation->set_rules('cellno', 'Cell No', 'required|max_length[50]');
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($role=='admin')
			{
				...
				$this->load->view('donation/donation',$data1);
		...
		$this->donation_model->donation_post($data);
		
		//Loading View
		$message['success_msg'] = '<CENTER> <h3 style="color:green;">You have successfully submitted the donation detail. </h3></CENTER><br>';
		...
		if($role=='admin')
			{
				...
				$this->load->view('profile_header');
				$this->load->view('admin/admin_left_menu');
				$this->load->view('donation/donation');
				$this->load->view('footer.php');
			...
			else
			{
				...
				$this->load->view('header',$data);				
				$this->load->view('donation/donation_left_menu');
				$this->load->view('donation/donation',$message);
				$this->load->view('footer.php');
			}
		}
