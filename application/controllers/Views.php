	  ...
	  if (empty($_FILES['sfile']['name']))
		...
		else
		{
			...
			if ( ! $this->upload->do_upload('sfile'))
				{
				$this->load->view('story/success_story',array('error' => $this->upload->display_errors('<div class="error">', '</div>')));
	...
	function view_detail($id)  
	 {
		 ...
		 if($id!='')
		 {
			$view_detail=$this->Views_model->view_detail($id);
			$data=$this->header();
			...
			if($role=='admin')
			{
				...
				$this->load->view('alumni_views/view_detail', array('view_detail'=>$view_detail));
				$this->load->view('footer.php');				
			}
			else
			{// it shows to every one
				$this->load->view('header',$data);
				$this->load->view('alumni_views/views_left_menu',$data1);
				$this->load->view('alumni_views/view_detail', array('view_detail'=>$view_detail));
				$this->load->view('footer.php');
			}	 
	 ...
	 function get_all_views($record_limit)
	 {
		...
		$views_count=$this->Views_model->views_count();
		$data=$this->header();
		$data1=$this->search_left_menu();	
		...
		//it fetch the record 
		$views_list=$this->Views_model->get_all_views($record_limit);	
		//print_r($newsevent_list);
		$data2=$this->pagination($views_count);	
		$data2['views_list']=$views_list;		
		/////////////////////////////////
		
		$this->load->view('header',$data);
		$this->load->view('alumni_views/views_left_menu',$data1);
		$this->load->view('alumni_views/view_list', $data2);	
		$this->load->view('footer.php'); 
